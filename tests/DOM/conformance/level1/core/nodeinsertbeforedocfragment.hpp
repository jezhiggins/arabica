#ifndef test_nodeinsertbeforedocfragment
#define test_nodeinsertbeforedocfragment
...
class nodeinsertbeforedocfragment : public DOMTestCase<string_type, string_adaptor> 
{
    typedef DOMTestCase<string_type, string_adaptor> baseT;
   public: 
     nodeinsertbeforedocfragment(std::string name) : baseT(name)
     {
  ...
  typedef bool boolean;
   ...
   void runTest()
   {
      ...
      baseT::assertEquals("newChild1", childName, __LINE__, __FILE__);
  child = childList.item(4);
      childName = child.getNodeName();
      baseT::assertEquals("newChild2", childName, __LINE__, __FILE__);
