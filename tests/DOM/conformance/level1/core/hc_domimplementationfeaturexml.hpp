#ifndef test_hc_domimplementationfeaturexml
#define test_hc_domimplementationfeaturexml
...
class hc_domimplementationfeaturexml : public DOMTestCase<string_type, string_adaptor> 
{
    typedef DOMTestCase<string_type, string_adaptor> baseT;
   public: 
     hc_domimplementationfeaturexml(std::string name) : baseT(name)
     {
  ...
  typedef bool boolean;
   ...
   void runTest()
   {
      ...
      if (("text/html" == baseT::getContentType())) {
          state = domImpl.hasFeature(SA::construct_from_utf8("html"), SA::construct_from_utf8("1.0"));
assertTrue(state);
      } else {
