#ifndef test_hc_nodecommentnodevalue
#define test_hc_nodecommentnodevalue
...
class hc_nodecommentnodevalue : public DOMTestCase<string_type, string_adaptor> 
{
    typedef DOMTestCase<string_type, string_adaptor> baseT;
   public: 
     hc_nodecommentnodevalue(std::string name) : baseT(name)
     {
  ...
  typedef bool boolean;
   ...
   void runTest()
   {
    ...
    commentName = commentNode.getNodeName();
      ...
      commentValue = commentNode.getNodeValue();
      baseT::assertEquals(" This is a comment", commentValue, __LINE__, __FILE__);
